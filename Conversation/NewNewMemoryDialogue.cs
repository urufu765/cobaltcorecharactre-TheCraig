...
internal class NewNewMemoryDialogue : IRegisterable
{
        ...
        LocalDB.DumpStoryToLocalLocale("en", new Dictionary<string, DialogueMachine>()
        {
            ...
            {"Illeana_Memory_3", new(){
                ...
                dialogue = [
                    ...
                    new(new Wait{secs = 3}),
                    new(new BGAction{action = "setLatest$Systems rebooting...12%"}),
                    new(new Wait{secs = 1}),
                    new(new BGAction{action = "setLatest$Systems rebooting...40%"}),
                    new(new Wait{secs = 0.3}),
                    new(new BGAction{action = "setLatest$Systems rebooting...97%"}),
                    new(new Wait{secs = 1}),
                    new(new BGAction{action = "setLatest$Systems rebooting...100%"}),
                    new(new BGAction{action = ">>>Systems OK!"}),
                    new(new Wait{secs = 1}),
                    new(new BGAction{action = ">*>Resuming <c=boldPink>CORE_craig</c>.$Resuming <c=boldPink>CORE_craig</c>..$Resuming <c=boldPink>CORE_craig</c>..."}),
                    new(new Wait{secs = 7}),
                    new(new BGAction{action = "setAndClearLatest$Resuming <c=boldPink>CORE_craig</c>..."}),
                    new(new BGAction{action = ">*>Load failed! <c=redd>Error:</c>$Load failed! <c=redd>Error: CORRUPTED_KEY_HEADER(E_00079)</c>"}),
                    new(new BGAction{action = "blinkInterval$0.333"}),
                    new(new Wait{secs = 1}),
                    new(new BGAction{action = ">>>Entering auto recovery"}),
                    new(new BGAction{action = "animationInterval$1.5"}),
                    new(new BGAction{action = ">+>............"}),
                    new(new Wait{secs = 5}),
                    new(new BGAction{action = "clear"}),
                    new(new Wait{secs = 2}),
                    new(new BGAction{action = ">*>Fetching recovery drive.../$Fetching recovery drive...-$Fetching recovery drive...\\$Fetching recovery drive... |"}),
                    new(new BGAction{action = "blinkInterval$0.1"}),
                    new(new Wait{secs = 4}),
                    new(new BGAction{action = "setAndClearLatest$Fetching recovery drive...Fail"}),
                    new(new BGAction{action = ">*>Fetch failed! <c=redd>Error:</c>$Fetch failed! <c=redd>Error: MISSING_DRIVE(E_01007)</c>"}),
                    new(new BGAction{action = "blinkInterval$0.333"}),
                    new(new Wait{secs = 1}),
                    new(new BGAction{action = ">*>Fetching backups <c=textFaint>(LOCAL)</c>.../$Fetching backups <c=textFaint>(LOCAL)</c>...-$Fetching backups <c=textFaint>(LOCAL)</c>...\\$Fetching backups <c=textFaint>(LOCAL)</c>... |"}),
                    new(new BGAction{action = "blinkInterval$0.1"}),
                    new(new Wait{secs = 5}),
                    new(new BGAction{action = "setAndClearLatest$Fetching backups <c=textFaint>(LOCAL)</c>...Success"}),
                    new(new BGAction{action = ">>>Found <c=boldPink>14</c>. Attempt loading in <c=textFaint>Ascending</c> order"}),
                    new(new BGAction{action = ">+>............"}),
                    new(new Wait{secs = 3}),
                    new(new BGAction{action = "clear"}),
                    new(new Wait{secs = 2}),
                    new(new BGAction{action = ">*>Attempting to load <c=boldPink>CORE_0lizr</c>.$Attempting to load <c=boldPink>CORE_0lizr</c>..$Attempting to load <c=boldPink>CORE_0lizr</c>..."}),
                    new(new Wait{secs = 7}),
                    new(new BGAction{action = "setAndClearLatest$Attempting to load <c=boldPink>CORE_0lizr</c>..."}),
                    ...
                    // Change AmIlleana to AmAlmostIlleana, a hybrid of Illeana, Lisard, and Craig.
